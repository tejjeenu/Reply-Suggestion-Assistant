...
class AutomaticConversationMemory(context: Context) {
    ...
    fun append(conversationName: String, visibleText: String): AutomaticMemorySummary {
        ...
        if (!isDuplicate) {
            entries += MemoryEntry(System.currentTimeMillis(), text)
        }
        ...
        return runCatching {
            ...
            buildList {
                for (index in 0 until array.length()) {
                    ...
                    if (text.isNotBlank()) {
                        add(MemoryEntry(item.optLong("captured_at"), text))
                    }
                }
            }
    ...
    private data class MemoryEntry(val capturedAt: Long, val text: String)
